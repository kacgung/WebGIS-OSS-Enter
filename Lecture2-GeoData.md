...
2. '레이어 > 속성 > 원본' 에서 각 레이별로 올바른 좌표계를 정의해주어야 합니다. 본 실습에서 지적도는 5174, 지형도는 5186입니다. 
...
> 다음 내용은 `PROJ 4` 라이브러리를 사용하는 QGIS 사용 시 해당됩니다. `PROJ 6` 이상을 사용하는 `QGIS 3.10.3` 이상에서는 해당사항 없습니다.<br><br>
국토지리정보원의 공식 "국가좌표변환계수"는 Molodensky-Badekas 모델용 10개 변수인데 `PROJ 4` 는 Bursa-Wolf 모델용 7개 변수를 지원해서 아래와 같은 조치가 필요합니다.

<br>

우리가 만날 수 있는 아래와 같은 경우에 위치 맞추기를 해 보겠습니다.

![](img/2023-01-28-20-03-36.png)

베셀 타원체 좌표계를 사용하는 경우 수백미터 정도 위치가 틀리는 문제가 국내 자료에서 많이 생깁니다. 
우리가 받은 지적도를 관리하는 KLIS 시스템이 EPSG:5174라는 좌표계를 사용중인데, EPSG:5174 는 Bessel 타원체입니다.

관련 조치에 참고하기 위해 다음 웹 페이지로 가 보겠습니다. https://www.osgeo.kr/17

웹페이지에서 5174를 검색해 보면 좌표계 변환 계수 정보를 찾을 수 있습니다.
+towgs84 을 적용하는 것입니다.
